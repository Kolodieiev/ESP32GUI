...
int UnpackFrameHeader(unsigned char *buf)
{
    ...
    if (m_FrameHeader->brIdx)
    {
        ...
        m_MP3DecInfo->nSlots = (int)slotTab[m_MPEGVersion][m_FrameHeader->srIdx][m_FrameHeader->brIdx] - (int)sideBytesTab[m_MPEGVersion][(m_sMode == Mono ? 0 : 1)] - 4 - (m_FrameHeader->crc ? 2 : 0) + (m_FrameHeader->paddingBit ? 1 : 0);
    }
...
int MP3FindSyncWord(unsigned char *buf, int nBytes)
{
    ...
    {
        if ((buf[i + 0] & m_SYNCWORDH) == m_SYNCWORDH && (buf[i + 1] & m_SYNCWORDL) == m_SYNCWORDL)
            return i;
    }
...
void MP3GetLastFrameInfo()
{
    ...
    else
    {
        ...
        m_MP3FrameInfo->outputSamps = m_MP3DecInfo->nChans * (int)samplesPerFrameTab[m_MPEGVersion][m_MP3DecInfo->layer - 1];
...
bool MP3Decoder_AllocateBuffers(void)
{
    if (!m_MP3DecInfo)
    {
        m_MP3DecInfo = (MP3DecInfo_t *)malloc(sizeof(MP3DecInfo_t));
    }
    if (!m_FrameHeader)
    {
        m_FrameHeader = (FrameHeader_t *)malloc(sizeof(FrameHeader_t));
    }
    if (!m_SideInfo)
    {
        m_SideInfo = (SideInfo_t *)malloc(sizeof(SideInfo_t));
    }
    if (!m_ScaleFactorJS)
    {
        m_ScaleFactorJS = (ScaleFactorJS_t *)malloc(sizeof(ScaleFactorJS_t));
    }
    if (!m_HuffmanInfo)
    {
        m_HuffmanInfo = (HuffmanInfo_t *)malloc(sizeof(HuffmanInfo_t));
    }
    if (!m_DequantInfo)
    {
        m_DequantInfo = (DequantInfo_t *)malloc(sizeof(DequantInfo_t));
    }
    if (!m_IMDCTInfo)
    {
        m_IMDCTInfo = (IMDCTInfo_t *)malloc(sizeof(IMDCTInfo_t));
    }
    if (!m_SubbandInfo)
    {
        m_SubbandInfo = (SubbandInfo_t *)malloc(sizeof(SubbandInfo_t));
    }
    if (!m_MP3FrameInfo)
    {
        m_MP3FrameInfo = (MP3FrameInfo_t *)malloc(sizeof(MP3FrameInfo_t));
    }
...
#ifdef IS_DEBUG
        Serial.println(F("not enough memory to allocate mp3decoder buffers"));
#endif
        return false;
    }
