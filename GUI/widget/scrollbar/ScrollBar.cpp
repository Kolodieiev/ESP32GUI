...
ScrollBar::ScrollBar(uint16_t widget_ID, TFT_eSPI &tft) : IWidget(widget_ID, tft) {}
...
void ScrollBar::onDraw()
{
    ...
    uint16_t slider_x_pos = _x_pos;
    uint16_t slider_y_pos = _y_pos;

    uint16_t step_value = _smart_scroll_enabled ? _smart_value : _cur_value;

    if (_orientation == ORIENTATION_VERTICAL)
        slider_y_pos = _y_pos + _slider_step_size * step_value;
    else
        slider_x_pos = _x_pos + _slider_step_size * step_value;

    // fill body
    _tft.fillRect(_x_pos, _y_pos, _width, _height, _back_color);

    // clear old
    if (_slider_last_x_pos < _x_pos)
        _slider_last_x_pos = _x_pos;
    if (_slider_last_y_pos < _y_pos)
        _slider_last_y_pos = _y_pos;

    _tft.fillRect(_slider_last_x_pos, _slider_last_y_pos, _slider_width, _slider_height, _back_color);

    _slider_last_x_pos = slider_x_pos;
    _slider_last_y_pos = slider_y_pos;

    // draw new
    _tft.fillRect(slider_x_pos, slider_y_pos, _slider_width, _slider_height, _slider_color);
}
